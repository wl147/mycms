...
namespace SiteServer.BackgroundPages.Core
{
    public class WebUtils
    {
        ...
        private static string GetContentEditUrlForExamination(EContentModelType modelType, int publishmentSystemId, int nodeId, int id, string returnUrl)
        {
            if (modelType == EContentModelType.GovPublic)
            {
                return PageGovPublicContentAdd.GetRedirectUrlOfEdit(publishmentSystemId, nodeId, id, returnUrl);
            ...
            return PageContentAdd.GetRedirectUrlOfEditForExamiantion(publishmentSystemId, nodeId, id, returnUrl);
        }
        ...
        public static string GetContentAddEditUrlForExamination(int publishmentSystemId, NodeInfo nodeInfo, int id, string returnUrl)
        {
            var modelType = EContentModelTypeUtils.GetEnumType(nodeInfo.ContentModelId);
            return GetContentEditUrlForExamination(modelType, publishmentSystemId, nodeInfo.NodeId, id, returnUrl);
        }
        ...
        public static string GetContentCommandsStandardForExamination(string administratorName, PublishmentSystemInfo publishmentSystemInfo, NodeInfo nodeInfo, string pageUrl, string currentFileName, bool isCheckPage,int ArticleId)
        {
            var iconUrl = SiteServerAssets.GetIconUrl(string.Empty);
            var modelType = EContentModelTypeUtils.GetEnumType(nodeInfo.ContentModelId);
            ...
            if (!isCheckPage && AdminUtility.HasChannelPermissions(administratorName, publishmentSystemInfo.PublishmentSystemId, nodeInfo.NodeId, AppManager.Cms.Permission.Channel.ContentAdd) && nodeInfo.Additional.IsContentAddable)
            {
                var redirectUrl = GetContentAddEditUrlForExamination(publishmentSystemInfo.PublishmentSystemId, nodeInfo, ArticleId, pageUrl);
                var title = "添加内容";
                if (modelType == EContentModelType.GovPublic)
                {
                    title = "采集信息";
...
";
        ...
        public static string GetContentType(string str)
        {
            ...
            if (str.Equals("Review")) return "PageContentReview";
            if (str.Equals("Examination")) return "PageExamination";
            return retval;
